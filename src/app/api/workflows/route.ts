...
export const GET = handler(async (req, user) => {
  ...
  const { rows } = await query(
    `SELECT w.id, w.tag, w.title, w.client_label, w.is_public, w.flagged_stale, w.updated_at, w.shared_from,
     ...
     FROM workflows w LEFT JOIN users u ON u.id = w.owner_id
...
export const POST = handler(async (req, user) => {
  const { tag, title, description, client_label } = await req.json();
  ...
  const { rows } = await query(
    `INSERT INTO workflows (owner_id, is_public, tag, title, description, client_label)
     VALUES ($1, FALSE, $2, $3, $4, $5) RETURNING *`,
    [user.id, t, title ?? t, description ?? '', client_label ?? null],
  );
